...
export const updateUser = async (req, res) => {
    ...
    try {
        ...
        if (profileImg) {
            if (user.profileImg) {
                await cloudinary.uploader.destroy(user.profileImg.split("/").pop().split(".")[0]); //xoá ảnh bằng cách xoá id của ảnh ở cuối url, bỏ .png
            }
        ...
        if (coverImg) {
            if (user.coverImg) {
                await cloudinary.uploader.destroy(user.coverImg.split("/").pop().split(".")[0]); //xoá ảnh bằng cách xoá id của ảnh ở cuối url, bỏ .png
            }
